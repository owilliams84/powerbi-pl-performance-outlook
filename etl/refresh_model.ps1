...
EVALUATE
ROW(
    "Fact rows", COUNTROWS(Financials),
    "Accounts", COUNTROWS(Account),
    "Units", COUNTROWS('Business Unit'),
    "Actual 2020", FORMAT(CALCULATE(SUM(Financials[Amount]), Financials[Scenario] = "Actual", 'Date'[Year] = 2020), "#,0")
)
"@
