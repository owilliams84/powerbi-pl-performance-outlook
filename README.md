...
The report carries the Milestone BI palette - near-black indigo `#0A0917`, gold `#C9A227`, and the
site's greys - with the brand mark in a band across the top of the page, so it reads as the same
studio's work as the other reports. The mark is a registered image: the SVG lives in
`StaticResources/RegisteredResources/`, is listed in `report.json` under `resourcePackages` with
`"type": "Image"`, and binds to an `image` visual through a `ResourcePackageItem` expression rather
than a path.

Two things worth knowing if you restyle it. The theme applies a white card background and a
hairline border to **every** visual, textboxes included - so a textbox meant to sit on the dark
band needs `background: show=false` declared explicitly, or its white text lands on a white box and
...
The statement at
**[milestonebi.com/pl-performance-outlook](https://www.milestonebi.com/pl-performance-outlook/)**
is this report rebuilt as hand-written HTML and SVG, including the three-view switcher. It reads
`web/pl-performance-outlook.json`, generated by `etl/build_web_data.py` with the same account
mapping, the same display-sign rule and the same YTD window as the DAX measures, so the two agree
to the thousand.

That JSON **is** committed, and the ledger still is not. What it holds is the 21-line statement
already printed in this README's verification block and visible in the screenshot above, revenue by
month, and EBITDA variance across seven territories - an aggregate summary of a 27,909-line ledger,
with no GL posting and no account-level figure in it.
...
the project and nothing else needs changing. The value committed here is the author's own clone;
it will not exist on your machine.
